...
if not DEFAULT_OLLAMA_MODEL:
    DEFAULT_OLLAMA_MODEL = "deepseek-coder:latest" # Fallback to a very common default
...
def get_ollama_response(prompt, history, selected_model, selected_role):
    """
    Function to interact with the Ollama API.
    Now accepts 'selected_model' and 'selected_role' as arguments.
    """
    messages = []

    # Add a system message based on the selected role
    if selected_role == "Engineer":
        system_message_content = "You are an experienced software engineer. Provide practical, efficient, and well-documented solutions."
    elif selected_role == "Architect":
        system_message_content = "You are a seasoned software architect. Focus on high-level design, scalability, maintainability, and system integration."
    elif selected_role == "Coder":
        system_message_content = "You are a skilled coder. Provide concise, correct, and idiomatic code snippets, focusing on implementation details."
    else: # Default or None selected
...
with gr.Blocks(theme="soft", title="Ollama Gradio Chatbot", css=app_css) as chatbot_ui:
    ...
    with gr.Row(elem_id="main_layout_row"):
        ...
        with gr.Column(elem_id="right_controls_column"):
            ...
            role_state = gr.State(value="Default") # New state for the role
            ...
            role_dropdown = gr.Dropdown(
                choices=["Default", "Engineer", "Architect", "Coder"],
                value="Default",
                label="Select AI Role",
                interactive=True,
                elem_id="role_dropdown_component"
            )
        ...
        def bot_response(history, selected_model_value, selected_role_value):
            ...
            bot_message = get_ollama_response(user_message, ollama_history, selected_model_value, selected_role_value)
            history[-1][1] = bot_message
            return history

        # Update model_state when model dropdown changes
        model_dropdown.change(fn=lambda x: x, inputs=model_dropdown, outputs=model_state)
        # Update role_state when role dropdown changes
        role_dropdown.change(fn=lambda x: x, inputs=role_dropdown, outputs=role_state)
        ...
        msg.submit(
            ...
            bot_response, 
            [chatbot, model_state, role_state], # Added role_state here
            chatbot
        )
        submit_btn.click(
            user_message, 
        ...
        def retry_last_message(history, selected_model_value, selected_role_value): # Added selected_role_value
            if not history:
                return []
            ...
            return bot_response(processed_history + [[last_user_message, None]], selected_model_value, selected_role_value) # Added selected_role_value

        retry_btn.click(
            retry_last_message, [chatbot, model_state, role_state], chatbot # Added role_state
        )
